...
def ParseCommandLine():
    ...
    parser.add_argument('-d', '--destination', dest='Destination', help='Output destination.  Where to place all result output, whether to prompt or a file or null for no output', default='prompt')
    parser.add_argument('-e', '--export', dest='Export', help='Type to export to, whether CSV or JSON', required=False)
    ...
    parser.add_argument('--diff-actions', dest='DiffActions', help='The diff actions to run, can be several together: codes, msgs, types, all', nargs='*', default=[])
    parser.add_argument('--validate-actions', dest='ValidateActions', help='The validate actions to run, can be several together: module, msg, hasmsg, all', nargs='*', default=[])
    ...
    CheckValidArg(arguments.Destination, Destinations.Destinations.Values())
    if(arguments.Export is not None):
        CheckValidArg(arguments.Export, Destinations.Exports.Values())

    CheckValidArgs(arguments.DiffActions, ActionTypes.DiffActions.Values())
